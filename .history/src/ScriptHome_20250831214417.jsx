...
export function useInvitadosConfig() {
  ...
  const invitadosFiltrados = invitados.filter((i) => {
    ...
    const coincideBusqueda = i.nombre.toLowerCase().includes(buscar.toLowerCase());
    return coincideFiltro && coincideBusqueda;
  });

  const totalPaginas = Math.ceil(invitadosFiltrados.length / registrosPorPagina);
  const indiceInicial = (paginaActual - 1) * registrosPorPagina;
  const invitadosPagina = invitadosFiltrados.slice(indiceInicial, indiceInicial + registrosPorPagina);
