...
export function useInvitadosConfig() {
  ...
  const totalPaginas = Math.ceil(invitados.length / registrosPorPagina);

  const invitadosPaginados = invitados.slice(
  ...
  const handleAgregarInvitado = () => {
    if (nuevoInvitado.nombre && nuevoInvitado.email) {
      setInvitados((prev) => [
        ...prev,
        { id: Date.now(), ...nuevoInvitado },
      ]);
      setNuevoInvitado({ nombre: "", email: "", estado: "pendiente" });
